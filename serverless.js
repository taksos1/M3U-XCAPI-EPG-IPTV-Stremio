...
const createAddon = require("./enhanced-addon");
...
const PREFETCH_MAX_BYTES = parseInt(
  process.env.PREFETCH_MAX_BYTES || "50000000",
  10,
);
...
app.use("/:token", async (req, res, next) => {
  ...
  if (!iface) {
    try {
      dlog("Building enhanced addon interface (cache miss)", ifaceKey);
      iface = await createAddon(config);
      if (CACHE_ENABLED) {
        interfaceCache.set(ifaceKey, iface);
      }
    } catch (e) {
      console.error("[SERVERLESS] Enhanced addon build failed:", e);
      return res.status(500).json({ error: "Enhanced addon build error" });
    }
  } else {
