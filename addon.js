...
class M3UEPGAddon {
    ...
    buildGenresInManifest() {
        ...
        if (movieCatalog) {
            // Count items per category to prioritize popular categories
            const movieCategoryCounts = {};
            this.movies.forEach(movie => {
                const cat = (movie.category || movie.attributes?.['group-title'] || '').trim();
                if (cat && cat.length > 0 && cat.length < 100) {
                    movieCategoryCounts[cat] = (movieCategoryCounts[cat] || 0) + 1;
                }
            });
            
            // Sort categories by count (descending) then alphabetically
            const movieGroups = Object.keys(movieCategoryCounts)
                .sort((a, b) => {
                    const countDiff = movieCategoryCounts[b] - movieCategoryCounts[a];
                    return countDiff !== 0 ? countDiff : a.localeCompare(b);
                });
                
            movieCatalog.genres = movieGroups;
            this.log.debug('Movie categories found:', {
                totalMovies: this.movies.length,
                uniqueCategories: movieGroups.length,
                topCategories: movieGroups.slice(0, 10).map(cat => `${cat} (${movieCategoryCounts[cat]})`),
                allCategories: movieGroups
            });
        }

        if (seriesCatalog) {
            // Count items per category to prioritize popular categories
            const seriesCategoryCounts = {};
            this.series.forEach(series => {
                const cat = (series.category || series.attributes?.['group-title'] || '').trim();
                if (cat && cat.length > 0 && cat.length < 100) {
                    seriesCategoryCounts[cat] = (seriesCategoryCounts[cat] || 0) + 1;
                }
            });
            
            // Sort categories by count (descending) then alphabetically
            const seriesGroups = Object.keys(seriesCategoryCounts)
                .sort((a, b) => {
                    const countDiff = seriesCategoryCounts[b] - seriesCategoryCounts[a];
                    return countDiff !== 0 ? countDiff : a.localeCompare(b);
                });
                
            seriesCatalog.genres = seriesGroups;
            this.log.debug('Series categories found:', {
                totalSeries: this.series.length,
                uniqueCategories: seriesGroups.length,
                topCategories: seriesGroups.slice(0, 10).map(cat => `${cat} (${seriesCategoryCounts[cat]})`),
                allCategories: seriesGroups
            });
        }
    ...
    getStream(id) {
        ...
        if (id.startsWith('iptv_series_ep_')) {
            ...
            // Find the series this episode belongs to for bingeGroup
            let seriesId = null;
            for (const [sId, info] of this.seriesInfoCache.entries()) {
                if (info && Array.isArray(info.videos)) {
                    const found = info.videos.find(v => v.id === id);
                    if (found) {
                        seriesId = sId;
                        break;
                    }
            ...
            // Fallback: check direct series episode index
            if (!seriesId) {
                for (const [sId, episodes] of this.directSeriesEpisodeIndex.entries()) {
                    const found = episodes.find(v => v.id === id);
                    if (found) {
                        seriesId = sId;
                        break;
                    }
            ...
            // Add bingeGroup if we found the series - this maintains server selection across episodes
            if (seriesId) {
                behaviorHints.bingeGroup = `iptv_series_${seriesId}`;
            }
